...
\begin{abstract}
A network must be large enough before communities emerge. The smallest size at which the split wins is the Ramsey community number
$\rk$. For a family of networks grown by a fixed
...
$s$ edges, and acts on the statistics the two models depend on as an exact
...
difficulty is that any sufficiently flexible algorithm will happily carve
communities out of a structureless graph, so the existence of a partition
...
$\rk\approx35$~\cite{vazquez2026ringwantsbroken}; on the scale-free
pseudofractal web of Dorogovtsev, Goltsev and Mendes~\cite{dgm2002} the reported
value drops from $1095$ to $42$ when the comparison is made against a
degree-corrected null~\cite{karrer2011,
vazquez2026communitystructurepseudofractalweb}. In every case reported so far,
...
Kadanoff~\cite{migdal1976,kadanoff1976}, the classic arena in which real-space
...
of Sec.~\ref{sec:evidence} worth making.
...
the bundles meet only at the poles, block~1 is internally complete and every
cross edge is an edge of bundle~1 incident on a pole. Elementary counting then
...
become ever more cleanly separated as the lattice grows. Note also that the cut
is nearly balanced in both node number and degree,
$\kappa_1/\kappa_2=1+2(1/s)^t+O(s^{-2t})\to1$, a fact that will matter in
...
what the present calculation shows is that the separation must be attributed to
whichever of the two effects---degree correction or the labelling prior---is
actually responsible, and that on the diamond it is entirely the latter. (ii) The
entire gap between the ``plain'' and ``degree-corrected'' curves as previously
reported is $\Lambda_t$. In the labelled convention both densities converge to
\begin{equation}
...
there~\cite{vazquez2026communitystructurepseudofractalweb}. That factor is thus
a property of the node-to-edge ratio and of the labelling prior, not of degree
correction, and it is universal in this precise sense. All statements below use
...
\begin{table}[tb]
  ...
  included; the fifth column is the degree-corrected evidence with the prior
  added, and matches the sixth to $O(\ln m)$.}
...
density $\ln3$~\cite{vazquez2026communitystructurepseudofractalweb}. By the
\emph{detection problem} we mean, throughout, the comparison of the two models
...
The approach to $\ln K$ is thus not merely observed but accounted for, term by
term, by the seam mode of the RG map: the deficit is the price of the $b^t$
...
within a few generations and the extensive $\ln K$ term takes over. Prior
...
misfire window has relative width $\delta_{t_\star}\sim
...
during the transient, i.e.\ by the moment the split first repays its own
complexity. The exact rule Eq.~\eqref{eq:cross} handles this without
...
It is worth separating two things that the literature so far has treated
together. The first reading of $\rk$ is \emph{the crossing}: the smallest
...
$\rk$ it is quantitatively wrong in exactly the regime that matters.
...
Their competition is exactly what the resolution $\gamma$ controls.
...
optimal cut is thus at \emph{half the depth of the lattice}: the geometric
midpoint of its own hierarchy, a statement that involves neither $n$ nor $m$
and is the invariant content of the result. The corresponding number of
communities is
\begin{equation}
...
with the network, without bound, though more slowly than the linear dimension.
...
selection of Secs.~\ref{sec:gs} and~\ref{sec:hier}. We state this explicitly to
avoid any suggestion that the infinite-range term has been decimated---it has
not, and it need not be, since it does not move $T_c$.
...
One point deserves care here, because the numerics invite a wrong reading. The
order parameter $\rho_K(T)$ does steepen into a step as the lattice grows, for
every $K$ [Fig.~\ref{fig:potts}(a)], and it is tempting to take the steepening
at $K>2$ for the first-order transition familiar from the Potts model. It is
not. Two exact diagnostics show that the transition is continuous for every $K$
on this lattice:
\begin{enumerate}
...
\end{enumerate}
The physical conclusions of the hierarchy are unaffected: below $T_c(K)$ the
colour field is rigid and the imposed partition survives thermal noise
completely as $n\to\infty$; above it the colours decorrelate. What changes is
the characterization of the transition, which is continuous, with a
$K$-dependent thermal eigenvalue and an exponent $\nu(K)$ that decreases from
$1.338$ at $K=2$ toward $1/2$ at large $K$.
...
community number is the generation at which the extensive evidence---driven by
the volume eigenvalue of the exact map $M$ and reaching the density $\ln K$ per
edge at the community fixed point---first clears the detection threshold. It is
therefore computed, not fitted; and we have shown in Sec.~\ref{sec:tworeadings}
where the crossing reading and the asymptotic-slope reading part company,
namely at every certainty a practitioner would use, where the slope estimate
underestimates $\rk$ by one or two generations because the evidence density is
still tens of percent short of $\ln K$. On the same lattice the
Reichardt--Bornholdt community Hamiltonian has an exact thermodynamic
community-ordered phase, in which the two hubs occupy opposite communities
below the ferromagnetic critical temperature and the order persists as
$n\to\infty$, so the emergent partition is not only detectable but
thermodynamically stable; its optimal refinement is a nested hierarchy cut at
half the depth of the lattice, whose levels become rigid one after another at
temperatures falling as $3/(2\ln K)$.

We have also clarified two comparisons that the earlier literature had
conflated. Degree correction and the node-labelling prior are different things,
and on this lattice---whose bundle cut is balanced in both node number and
degree---it is the labelling prior alone that separates the ``plain'' from the
``degree-corrected'' curve, at a cost of $n\ln K$ and a density ratio of
$(1-n/m)^{-1}$. And the sharpening of the community order parameter with size
is finite-size scaling at a continuous transition, at every number of
communities, rather than the onset of a first-order jump.
