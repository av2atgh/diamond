...
$k_BT_c/J\to3/(2\ln K)$ (Sec.~\ref{sec:pottsT}).

Two comparisons reported previously require correction. First, degree
correction and the node-labelling prior are distinct effects. The bundle cut of
this lattice is balanced in node number and in degree, so the plain and
degree-corrected evidences agree to $O(\ln m)$ and give the same $\rk$; what
separates them is the labelling prior alone, worth $n\ln K$, and the ratio of
the two evidence densities is $(1-n/m)^{-1}$ (Sec.~\ref{sec:priors}). Second,
the sharpening of the community order parameter with size is finite-size scaling
at a continuous transition, for every $K$, and not the onset of a first-order
jump (Sec.~\ref{sec:pottsT}).
